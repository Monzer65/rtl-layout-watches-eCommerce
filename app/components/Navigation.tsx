...
const Navigation = () => {
  return (
    <div className='flex flex-col lg:flex-row justify-between lg:items-center gap-2 px-8 py-2 shadow-lg relative text-xs sm:text-sm'>
      ...
      <nav className='flex items-center gap-2 mt-4 lg:mt-0 order-1 lg:order-2'>
        <Link
          href={"/"}
          className='flex gap-1 items-center border border-gray-200 rounded-md px-2 py-1 sm:px-4 sm:py-2'
        >
          <ArrowRightEndOnRectangleIcon className='h-3 w-3 sm:h-4 sm:w-4 md:h-6 md:w-6 text-gray-500' />
          ورود | ثبت نام
        </Link>
        <Link
          href={"/"}
        ...
        >
          ...
          <ShoppingBagIcon className='h-3 w-3 sm:h-4 sm:w-4 md:h-6 md:w-6 text-gray-500' />
        </Link>
      </nav>
    </div>
  );
